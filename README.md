...
## Гараж и происхождение проекта

Компактный интерфейс в духе инди-гонки: таблица PAS рядом с настройками, расчётная приборка, переключение уровня помощи и асфальт/подъём. Анимация включается вручную, останавливается на скрытой вкладке и учитывает reduced motion. Светлая тема сохранена. Приборка показывает модель, а не телеметрию.

**BBS Flash построен на работе Bafang и Stefan Penov (Penoff).** Параметры, формат `.el` и UART-поведение взяты за основу из BafangConfigTool; программа Penoff, исходники и его справочник стали отправной точкой. Наш вклад — браузерный интерфейс, сравнение и расчёты, проверки записи и хранение копий. Кнопка `?` у каждого поля объясняет смысл, эффект и ограничения по справочнику Penoff. Это самостоятельная адаптация, без заявления об одобрении авторами оригинала.

[Оригинальный проект Penoff](https://penoff.me/2016/01/13/e-bike-conversion-software/) · [Источники и благодарности](docs/CREDITS.md)
...
- `src/garage.css`, `src/dash.js`, `src/guide.js` — оформление гаража, расчётная приборка и справочник Penoff RU/EN.
